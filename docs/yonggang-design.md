...
- 물리엔진은 `data/game-data.js`의 `physics.engine`, `physics.body`, `physics.drop`, `physics.merge`, `physics.gameOver` 설정을 기준으로 동작한다.
- 병합 직후 속도 전달과 상향 반동은 설정값으로 제한해 큰 티어가 과도하게 튀지 않도록 한다.
- 티어별 밀도/마찰/반발 계수는 유지하되, 공기저항은 반경 기반으로 보정해 큰 오브젝트가 더 안정적으로 가라앉게 한다.
